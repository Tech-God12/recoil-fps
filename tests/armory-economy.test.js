...
test('competent first run pays ≈ $3,350 and full unlock takes 15–21 runs', () => {
  ...
  // Band widened from $66k when the MCX Spear joined the catalog as the
  // flagship rifle. The guard's purpose is that a full unlock stays a couple of
  // dozen runs rather than a grind, and an eleventh weapon legitimately moves
  // the horizon by about a third of a run.
  assert.ok(catalogValue >= 55000 && catalogValue <= 70000, `catalog worth $${catalogValue}`);
