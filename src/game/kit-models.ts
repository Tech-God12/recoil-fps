...
export interface DartModel {
  ...
  led: THREE.Mesh;
  /** Flat expanding ring shown for each pulse; scaled by the director. */
  ring: THREE.Mesh;
}
...
export function buildBarricade(width: number, height: number, depth: number): BarricadeModel {
  ...
  for (let i = 0; i < 3; i++) {
    ...
    const p = box(pw - 0.02, h, depth, m.sand, x, h / 2, 0, group);
    p.receiveShadow = true;
    plates.push(p);
    box(pw - 0.06, 0.06, depth + 0.012, m.stripe, x, h - 0.12, 0, group);
    box(0.05, h * 0.9, depth + 0.02, m.steel, x - pw / 2 + 0.01, h * 0.47, 0, group); // hinge post
  }
  ...
  lamp.position.set(width / 2 - 0.12, height * 0.93 + 0.03, 0.04); group.add(lamp);
  return { group, plates, lamp };
}
...
export function buildDecoy(): DecoyModel {
  const m = M();
  const mat = new THREE.MeshBasicMaterial({ color: 0x6FE8FF, transparent: true, opacity: 0.55, blending: THREE.AdditiveBlending, depthWrite: false });
  const group = new THREE.Group();
  ...
  return { group, lLeg, rLeg, lArm, rArm, muzzle, mat, puck };
}
