...
export default function MissionObjective({ mission }: { mission: MissionHud }) {
  ...
  return (
    <>
      <section className={`objective-tracker ${warning ? 'objective-warning' : ''}`} aria-label="Current mission objective">
        <div className="objective-kicker"><span>{mission.name}</span><span>{String(mission.index + 1).padStart(2, '0')} / {String(mission.phaseCount).padStart(2, '0')}</span></div>
        <div className="objective-steps" aria-hidden="true">
          {Array.from({ length: mission.phaseCount }, (_, i) => <span key={i} className={i < mission.index ? 'done' : i === mission.index ? 'current' : ''} />)}
        </div>
        <h2>{mission.title}</h2>
        <p className="objective-brief">{mission.brief}</p>
        ...
        {(mission.type === 'clear' || mission.type === 'destroy') && <div className="objective-progress" role="progressbar" aria-label="Objective progress" aria-valuemin={0} aria-valuemax={100} aria-valuenow={Math.round(progress * 100)}><span style={{ width: `${progress * 100}%` }} /></div>}
        <div className="objective-location">
      ...
      {mission.waypoint.visible && mission.distance > mission.radius && <div className="objective-waypoint" style={{ left: `${mission.waypoint.x}%`, top: `${mission.waypoint.y}%` }} aria-hidden="true">
      ...
      {mission.canPlant && !mission.armed && <div className="mission-interact"><kbd>X</kbd><span>Hold to plant charge</span><span className="interact-progress" style={{ transform: `scaleX(${mission.plantProgress})` }} /></div>}
    </>
  );
}