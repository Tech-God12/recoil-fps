...
// Real Web Audio graph stand-in: every ability sound runs its actual synthesis code.
installAudioStub();
installCanvasStub();

const abilities = await import('../src/game/abilities.ts');
const { AbilityCharge, AbilityDirector, ABILITY_TUNING, ABILITY_DEFS, ABILITY_IDS, ABILITY_PRICES, ABILITY_LURE_BREAK_CHANCE, snapCardinal, barricadePlacement, sonarTagged, chooseLure, isAbilityId, recallRefundSeconds, burstVictims } = abilities;
const { buyAbility, equipAbility, readAbilities } = await import('../src/game/economy/ability-shop.ts');
const { DEFAULT_PROFILE, migrateProfile } = await import('../src/game/economy/profile.ts');
const { buildDart, buildBarricade, buildDecoy, buildMine, buildMedkit } = await import('../src/game/ability-models.ts');
...
const { AbilitySlot, AbilityEquipButton, AbilityPauseCard, AbilityHint, AbilityFx } = await import('../src/ui/Abilities.tsx');
const { default: AbilitiesMenu } = await import('../src/ui/AbilitiesMenu.tsx');
const { PauseMenu } = await import('../src/ui/Screens.tsx');

// ------------------------------------------------------------------ pure ----

test('exactly five abilities, each with a name, callout, price, steps, stats and a cooldown that matches the tuning table', () => {
  assert.deepEqual([...ABILITY_IDS], ['recon', 'bulwark', 'phantom', 'mine', 'medic']);
  assert.equal(Object.keys(ABILITY_DEFS).length, 5);
  assert.equal(ABILITY_DEFS.recon.cooldown, 45);
  assert.equal(ABILITY_DEFS.bulwark.cooldown, 60);
  assert.equal(ABILITY_DEFS.phantom.cooldown, 50);
  assert.equal(ABILITY_DEFS.mine.cooldown, 40);
  assert.equal(ABILITY_DEFS.medic.cooldown, 45);
  for (const id of ABILITY_IDS) {
    const d = ABILITY_DEFS[id];
    assert.ok(d.name && d.callout && d.blurb && d.rule, id);
    assert.equal(d.cooldown, ABILITY_TUNING[id].cooldown);
    assert.equal(d.price, ABILITY_PRICES[id]);
    assert.ok(d.price > 0);
  ...
  assert.equal(ABILITY_TUNING.killRefund, 0.08);
  assert.equal(ABILITY_TUNING.refundCap, 0.3);
  assert.equal(ABILITY_TUNING.deployCharge, 0.5);
  assert.ok(isAbilityId('phantom'));
  assert.ok(!isAbilityId('nuke'));
  assert.ok(!isAbilityId(3));
});

test('ability charge: spend gates on ready, ticks down, kill refunds are small and capped per charge', () => {
  const half = new AbilityCharge(50, 0.5);
  assert.equal(half.left, 25, 'a half-charged ability waits half a cooldown');
  assert.ok(!half.ready);
  const c = new AbilityCharge(30);
  assert.ok(c.ready);
  ...
  c.refund(ABILITY_TUNING.killRefund, ABILITY_TUNING.refundCap);
  assert.ok(Math.abs(c.left - 17.6) < 1e-9, 'a kill takes 2.4 s off a 30 s ability');
  for (let i = 0; i < 10; i++) c.refund(ABILITY_TUNING.killRefund, ABILITY_TUNING.refundCap);
...
test('sonar tags by true distance (through walls) and the lure picker wants sight and range', () => {
  ...
  assert.ok(ABILITY_LURE_BREAK_CHANCE > 0 && ABILITY_LURE_BREAK_CHANCE < 1);
});

test('abilities are bought, not given: shop rules, equip rules and save migration', () => {
  assert.deepEqual(DEFAULT_PROFILE.ownedAbilities, [], 'new players own no ability');
  assert.equal(DEFAULT_PROFILE.equippedAbility, null, 'and spawn without one');
  assert.ok(!('fieldAbility' in DEFAULT_SETTINGS), 'the old free settings ability is gone');
  assert.ok(!('fieldAbility' in sanitizeSettings('{"fieldAbility":"phantom"}')));
  const broke = { ...DEFAULT_PROFILE, cash: 1000 };
  assert.deepEqual(buyAbility(broke, 'recon'), { ok: false, error: 'INSUFFICIENT_FUNDS' });
  assert.deepEqual(buyAbility(broke, 'nuke'), { ok: false, error: 'UNKNOWN' });
  assert.deepEqual(equipAbility(broke, 'recon'), { ok: false, error: 'NOT_OWNED' });
  const rich = { ...DEFAULT_PROFILE, cash: 20000 };
  const a = buyAbility(rich, 'bulwark');
  assert.ok(a.ok);
  assert.equal(a.value.cash, 20000 - ABILITY_PRICES.bulwark);
  assert.deepEqual(a.value.ownedAbilities, ['bulwark']);
  assert.equal(a.value.equippedAbility, 'bulwark', 'the first ability bought is equipped');
  assert.deepEqual(buyAbility(a.value, 'bulwark'), { ok: false, error: 'ALREADY_OWNED' });
  const b = buyAbility(a.value, 'recon');
  assert.ok(b.ok);
  assert.equal(b.value.equippedAbility, 'bulwark', 'buying a second ability keeps the current one equipped');
  const c = equipAbility(b.value, 'recon');
  assert.ok(c.ok && c.value.equippedAbility === 'recon');
  const none = equipAbility(c.value, null);
  assert.ok(none.ok && none.value.equippedAbility === null, 'players can deploy with no ability');
  assert.equal(rich.cash, 20000, 'shop calls never mutate the input profile');
  // forged / legacy saves
  assert.deepEqual(readAbilities(['recon', 'nuke', 'recon', 7], 'phantom'), { ownedAbilities: ['recon'], equippedAbility: null });
  assert.deepEqual(readAbilities(undefined, 'recon'), { ownedAbilities: [], equippedAbility: null });
  const m = migrateProfile(JSON.stringify({ ...DEFAULT_PROFILE, ownedAbilities: ['phantom'], equippedAbility: 'phantom' }));
  assert.deepEqual([m.ownedAbilities, m.equippedAbility], [['phantom'], 'phantom']);
});
...
test('recall refund and burst victims are pure and bounded', () => {
  ...
  assert.deepEqual(burstVictims({ x: 0, z: 0 }, ABILITY_TUNING.phantom.burstRadius, pts).map(p => pts.indexOf(p)), [0, 2]);
});

test('mine damage: full inside the core, linear falloff to the edge, nothing beyond', () => {
  const { mineDamage } = abilities;
  const T = abilities.ABILITY_TUNING.mine;
...
test('ability hardware stays inside a small geometry budget', () => {
...
function fakeWorld() {
  ...
  const messages = [];
  const feedback = [];
  // Every Effects call is recorded; abilities reach sparks/slamDust/holoBurst/sonarPulse etc.
  const fxCalls = [];
...
test('Radar: flies, drops down the wall, sets up, pings three times, tags only what is in range, alerts, and cleans up', () => {
  const w = fakeWorld();
  const d = new AbilityDirector(w.ctx, 'recon', 1);
  ...
  assert.equal(d.damageMul(w.hostiles[0]), ABILITY_TUNING.recon.markDamageMul);
  ...
  assert.ok(w.alerts.every(a => a.radius === ABILITY_TUNING.recon.hearRadius));
...
test('Barricade: the wall registers as an occluder + blocker, soaks 450 HP of hostile fire, and is removed when broken', () => {
  const w = fakeWorld();
  const d = new AbilityDirector(w.ctx, 'bulwark', 1);
  const occ = w.occluders.length;
  assert.ok(d.activate());
  assert.equal(w.solids.length, 1);
  const plates = w.occluders.slice(occ);
  assert.equal(plates.length, 3);
  assert.ok(plates.every(p => typeof p.userData.abilityHit === 'function'));
  step(d, ABILITY_TUNING.bulwark.unfold + 0.05);
  ...
  assert.ok(hit && hit.object.userData.abilityHit, 'crouched player is covered');
  // hostile fire: 30 hits of 10.5 do not break it, the rest does
  for (let i = 0; i < 30; i++) plates[i % 3].userData.abilityHit(10.5);
  step(d, 0.1);
  assert.equal(d.liveCount, 1);
  assert.ok(Math.abs(d.hud().live[0].health - (450 - 315) / 450) < 1e-6);
  for (let i = 0; i < 13; i++) plates[i % 3].userData.abilityHit(10.5);
  step(d, 0.1);
...
test('Barricade: Z beside your wall recalls it and banks cooldown by remaining integrity', () => {
  const w = fakeWorld();
  const d = new AbilityDirector(w.ctx, 'bulwark', 1);
  assert.ok(d.activate());
  step(d, 1);
  ...
  w.ctx.occluders[1].userData.abilityHit(225); // half integrity
  assert.ok(d.activate(), 'recall works while the ability is recharging');
  ...
  assert.ok(w.messages.some(m => m.startsWith('BARRICADE PICKED UP')));
  step(d, ABILITY_TUNING.bulwark.fold + 0.05);
...
test('Barricade: refused placement keeps the charge, walls expire after 24 s, and hostile frags crack them', () => {
  const w = fakeWorld();
  const d = new AbilityDirector(w.ctx, 'bulwark', 1);
  w.solids.push({ minX: -5, maxX: 5, minZ: -3, maxZ: 0, minY: 0, maxY: 3 });
  assert.equal(d.activate(), false);
  assert.ok(d.charge.ready, 'a refused placement does not eat the ability');
  assert.ok(w.messages.some(m => m.includes('NO ROOM')));
  w.solids.length = 0;
  assert.ok(d.activate());
  step(d, ABILITY_TUNING.bulwark.life - 0.5);
  assert.equal(d.liveCount, 1);
  step(d, 1);
  ...
  step(d, ABILITY_TUNING.bulwark.cooldown);
  assert.ok(d.charge.ready);
  assert.ok(d.activate());
  step(d, ABILITY_TUNING.bulwark.unfold);
...
test('PHANTOM: the decoy runs ahead, fires audible blanks, lures sight-lines, dies to 120 damage and cleans up', () => {
  ...
  const d = new AbilityDirector(w.ctx, 'phantom', 1);
  const baseline = w.scene.children.length;
  assert.ok(d.activate());
  step(d, ABILITY_TUNING.phantom.runFor + 0.5);
  const lure = d.lures()[0];
  assert.ok(lure, 'decoy is live');
  const ran = lure.feet.x;
  assert.ok(ran > 8 && ran < 10.5, `ran ${ran.toFixed(2)} m`);
  assert.ok(w.alerts.length >= 2 && w.alerts.every(a => a.radius === ABILITY_TUNING.phantom.noiseRadius), 'blank fire is heard at 28 m');
...
test("Decoy: the decoy's death burst stuns hostiles within 6 m, even on timeout", () => {
  const w = fakeWorld();
  w.player.dir.set(1, 0, 0);
  const d = new AbilityDirector(w.ctx, 'phantom', 1);
  assert.ok(d.activate());
  step(d, ABILITY_TUNING.phantom.runFor + 0.2);
  const lure = d.lures()[0];
  ...
  step(d, ABILITY_TUNING.phantom.life);
  assert.equal(w.hostiles[3].stunned, ABILITY_TUNING.phantom.burstStun);
  assert.equal(w.hostiles[5].stunned, 0, '7 m is outside the burst');
  assert.ok(w.messages.includes('DECOY BURST — 1 ENEMY STUNNED'));
  assert.equal(d.stats.stunned, 1);
  d.dispose();
});

test('deploys half charged, locked ability, slow refunds, ready chime and onboarding hint', () => {
  const w = fakeWorld();
  const d = new AbilityDirector(w.ctx, 'recon');
  assert.ok(!d.charge.ready, 'no free ability at the spawn');
  assert.equal(d.charge.left, 22.5);
  assert.ok(d.hud().hint, 'fresh deployment shows the ability prompt');
  assert.equal(d.activate(), false);
  assert.ok(w.messages.some(m => m.startsWith('RADAR RECHARGING')));
  assert.equal(typeof d.buyAbility, 'undefined', 'no mid-match swapping');
  d.onKill(2);
  ...
  step(d, 60);
  assert.equal(d.activate(), false, 'dead players cannot use abilities');
  d.dispose();
});
...
test('mission AI: rounds that stop on a barricade plate damage it', () => {
  ...
  let soaked = 0;
  plate.userData.abilityHit = n => { soaked += n; };
  const damage = [];
...
test('HUD slot, prompt, fx, equipped-ability button, read-only pause card and ABILITIES menu all render', () => {
  const w = fakeWorld();
  const d = new AbilityDirector(w.ctx, 'bulwark', 1);
  const hud = d.hud();
  const slot = renderToStaticMarkup(React.createElement(AbilitySlot, { ability: hud }));
  ...
  const hint = renderToStaticMarkup(React.createElement(AbilityHint, { ability: hud }));
  assert.match(hint, /Press <span class="keycap">Z<\/span>/);
  for (const kind of ['ping', 'slam', 'burst', 'break', 'recall', 'decoy', 'ready']) {
    assert.match(renderToStaticMarkup(React.createElement(AbilityFx, { kind })), new RegExp(`ability-fx-${kind}`));
  }
  const none = renderToStaticMarkup(React.createElement(AbilityEquipButton, { ability: null, onOpen() {} }));
  assert.match(none, /NONE EQUIPPED/);
  assert.match(none, /GET A ABILITY/);
  const eq = renderToStaticMarkup(React.createElement(AbilityEquipButton, { ability: 'phantom', onOpen() {} }));
  assert.match(eq, /Decoy/);
  d.activate();
  const pause = renderToStaticMarkup(React.createElement(AbilityPauseCard, { ability: d.hud() }));
  assert.match(pause, /LOCKED FOR THIS DEPLOYMENT/);
  assert.doesNotMatch(pause, /SWAP|<button/, 'no way to change abilities mid-game');
  assert.match(renderToStaticMarkup(React.createElement(AbilityPauseCard, {})), /No ability/);
  const noop = () => {};
  const menu = renderToStaticMarkup(React.createElement(PauseMenu, { ability: d.hud(), onResume: noop, onRestart: noop, onSettings: noop, onQuit: noop }));
  for (const label of ['Resume', 'Settings', 'Restart', 'Quit to menu', 'ABILITY']) assert.match(menu, new RegExp(label));
  assert.doesNotMatch(menu, /SWAP/);
  d.dispose();
  // ABILITIES menu: three cards, the price on the buy button, the wallet on screen
  const shop = renderToStaticMarkup(React.createElement(AbilitiesMenu, { profile: { ...DEFAULT_PROFILE, cash: 5000 }, onProfile: noop, onBack: noop }));
  ...
  const owned = renderToStaticMarkup(React.createElement(AbilitiesMenu, { profile: { ...DEFAULT_PROFILE, ownedAbilities: ['recon'], equippedAbility: 'recon' }, onProfile: noop, onBack: noop }));
  assert.match(owned, /Equipped/);
  const poor = renderToStaticMarkup(React.createElement(AbilitiesMenu, { profile: { ...DEFAULT_PROFILE, cash: 100 }, onProfile: noop, onBack: noop }));
  assert.match(poor, /Need \$4,400 more/);
});

// ------------------------------------------------ engine wiring, real map ----

test('engine wiring on the real warehouse: placement checks, live collision, hittables and full ability lifecycle', async () => {
  ...
  // A bare engine carrying only what the ability bridge touches, with the real methods.
  ...
  const ctx = engine.abilityContext();
  const d = new AbilityDirector(ctx, 'bulwark', 1);
  ...
  assert.ok(p.z > open.z - ABILITY_TUNING.bulwark.placeDist, `walked to z=${p.z.toFixed(2)} from ${open.z}`);
